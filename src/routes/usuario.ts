    ...
    if (familiares && familiares.length > 0) {
      ...
      for (const familiar of familiares) {
        ...
        if (documentoExists.length > 0) {
          throw new Error(`El documento ${familiar.numeroDocumentoFamiliar} ya está registrado para otro familiar`);
        }
        ...
        console.log(`✅ Familiar registrado: ${familiar.nombreFamiliar} (${familiar.parentesco})`);
      }

      console.log(`✅ ${familiares.length} familiares procesados exitosamente`);
    } else {
      console.log("ℹ️  No hay familiares para procesar");
    }
